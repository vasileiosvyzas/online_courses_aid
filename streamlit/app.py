...
def main():
    ...
    if search:
        ...
        for index, hit in enumerate(response):
            ...
            st.write(search_result(i=index, url=f'https://www.udemy.com{hit.url}', title=hit.title, headline=hit.headline), unsafe_allow_html=True)
            st.subheader('Related YouTube videos')
            for i, video in enumerate(videos_list):
                st.write(search_result(i=i, url=f"https://www.youtube.com/{video['video']}", title=video['title'], headline=video['description']), unsafe_allow_html=True)
