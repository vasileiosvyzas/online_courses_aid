...
class UdemyClient:
    ...
    def request_data(self, url: str, current_session: Session) -> Dict:
        ...
        try:
            response = current_session.get(url=url, auth=HTTPBasicAuth(self.CLIENT_ID, self.CLIENT_SECRET), timeout=10)
    ...
    def get_course_details(self, course_id: str) -> Dict:
        ...
        single_courses_details = self.request_data(url=course_details_endpoint, current_session=session)
